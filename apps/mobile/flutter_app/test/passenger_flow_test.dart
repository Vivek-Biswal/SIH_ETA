...
class TestRepository implements TrainRepository {
  ...
  Future<JourneyResult> getJourney(String trainNumber, {String? date}) async {
    ...
    if (offline || noTrain) {
      final state = noTrain ? ApiResultStatus.notFound : ApiResultStatus.offline;
      return JourneyResult(ApiResult.error(noTrain ? 'Train not found' : 'Cannot reach server', status: state),
        ApiResult.error('Arrival information unavailable', status: state), DateTime.now());
    }
    return JourneyResult(ApiResult.success(TrainStatus.fromJson(statusJson())),
      etaOnlyFailure ? ApiResult.error('ETA service unavailable') :
        ApiResult.success(ETAModel.fromJson(etaJson())), DateTime.now());
  }
  @override
...
Future<void> pumpApp(WidgetTester tester, TestRepository repository, {String location = '/'}) async {
...
void main() {
  testWidgets('number lookup navigates to real fields with schedule-only labels', (tester) async {
    phone(tester);
  ...
  testWidgets('refresh retains stale data and recovers after connection failure', (tester) async {
    final repo = TestRepository();
  ...
  testWidgets('not found and direct-route back navigation are safe', (tester) async {
  ...
  testWidgets('ETA failure retains the backend status and route', (tester) async {
  ...
  testWidgets('station picker uses selected endpoints and displays empty results', (tester) async {
    ...
    await pumpApp(tester, repo);
    for (final pair in [('origin-picker', 'AAA'), ('destination-picker', 'CCC')]) {
  ...
  testWidgets('narrow phone with enlarged text has no overflow', (tester) async {
  ...
  testWidgets('loading is visible and overlapping refreshes are prevented', (tester) async {
    ...
    await tester.tap(find.byKey(const Key('refresh-journey')));
    expect(repo.calls, 1);
    repo.pending!.complete(JourneyResult(ApiResult.success(TrainStatus.fromJson(statusJson())),
      ApiResult.success(ETAModel.fromJson(etaJson())), DateTime.now()));
    await tester.pumpAndSettle();
    expect(find.text('Test Express'), findsOneWidget);
  });
